...
%     imshow(Z)
    ...
    s.info_3_extra(:,:,2) = get_parameters(features_3(info_3_extra).Image);

%     s.corrosive_3_extra(:,:,1) = get_parameters(features(corr_1).Image);
%     s.corrosive_3_extra(:,:,2) = get_parameters(features(corr_2).Image);
%     s.info_3_extra(:,:,1) = get_parameters(features_3(info).Image);
%     s.phone_3_extra(:,:,1) = get_parameters(features_2(phone_1).Image);
%     s.phone_3_extra(:,:,2) = get_parameters(features_2(phone_2).Image);
%    s.wc_extra_3_ch_2(:,:,1) = get_parameters(features_2(wc_3).Image);
