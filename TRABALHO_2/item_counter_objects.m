...
for i=1:num_files
    ...
    %Z = autobin(Z);
    %Z_bin = imbinarize(Z);
    %Z = bsxfun(@times,Z_bin,cast(Z, 'like', image));
    imshow(Z)
...
%     s.corrosive(:,:,3) = get_parameters(features(corrosive_3).Image);
% 
%     %electric
...
rotate_angles = linspace(0,359,10);
scale_factors = linspace(0.33,2,5);
%parameters = zeros(length(rotate_angles)*length(scale_factors),4);
...
for r = rotate_angles
    for s = scale_factors
        ...
        [~, idx] = max([all.Area]);
        invariant_moments = feature_vec(scaled_obj);
        %parameters(count,:) = [all(idx).Circularity all(idx).Solidity all(idx).Eccentricity invariant_moments(1)];
