...
num_circles_analyzing = 7;
num_squares_analyzing = 13;

num_circles_analyzing_2 = 4;
num_squares_analyzing_2 = 1;


num_circles_analyzing_1 = 1;
...
%dist_squares(:,toxic_2_s) = mahal(Patts_squares(:,[1 2 4 5 6 7]),s.toxic(:,[1 2 4 5 6 7],2));
...
%info 2 canal 2
info_2_c_2 = 1;
dist_squares_2(:,info_2_c_2) = mahal(Patts_squares_2,s.info(:,:,2));
...
minimos_circles_2  =transform_to_minims(dist_circles_2,num_circles_analyzing_2,threshold_c);
minimos_squares_2  =transform_to_minims(dist_squares_2,num_squares_analyzing_2,threshold_s);

minimos_circles_1  =transform_to_minims(dist_circles_1,num_circles_analyzing_1,threshold_c);

elec = sum(sum(minimos_triangles(:,[eletric_1_t eletric_2_t eletric_3_t])));
cor = sum(sum(minimos_triangles(:,[corrosive_1_t corrosive_2_t]))) + sum(minimos_squares(:,corrosive_3_s));
...
wc = sum(sum(minimos_squares(:,[wc_1_s wc_2_s]))) + sum(minimos_circles_2(:,wc_3_c_2));
wifi = sum(sum(minimos_squares(:,[wifi_2_s wifi_3_s])));
phone = sum(minimos_circles(:,phone_1_c))+sum(minimos_squares(:,phone_2_s)) + sum(minimos_circles_2(:,phone_3_c_2));
lit = sum(minimos_circles(:,lixo_1_c)) + sum(minimos_squares(:,lixo_3_s)) + sum(minimos_circles_2(:,lixo_2_c_2));
...
info = sum(minimos_circles(:,info_1_c)) + sum(minimos_squares_2(:,info_2_c_2)) + sum(minimos_circles_1(:,info_3_c_1));
    ...
    if sum(minimos_squares(object,:)) == 1
        for aj = 1:size(minimos_squares,2)
            if minimos_squares(object,aj)
                ...
                elseif aj == wifi_2_s || aj==wifi_3_s
    ...
    if sum(minimos_squares_2(object,:)) == 1
        for aj = 1:size(minimos_squares_2,2)
            if minimos_squares_2(object,aj)
                if aj==info_2_c_2
                    rectangle('Position', boxes_s_2(object,:),'EdgeColor','b','LineWidth',4);
                else
                    rectangle('Position', boxes_s_2(object,:),'EdgeColor','w','LineStyle','-.','LineWidth',4);
                end
            end
        end
    end
...
%                 else
%                     rectangle('Position', boxes_c(object,:),'EdgeColor','w','LineStyle','-.','LineWidth',4);
                end
            end
        end
    end
